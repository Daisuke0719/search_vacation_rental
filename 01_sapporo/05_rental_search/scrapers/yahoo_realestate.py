...
class YahooRealestateScraper(BaseScraper):
    ...
    def _build_search_url(self, building_name: str, ward: str) -> str:
        ...
        return (
            f"https://realestate.yahoo.co.jp/rent/search/"
            f"?prefecture=01&city=0101"
            f"&keyword={encoded}"
        )
    ...
    async def search(self, building_name: str, ward: str) -> list[ListingResult]:
        ...
        try:
            if not await self._safe_goto(page, url):
                logger.error(f"[yahoo] Failed to load for {building_name}")
                return results

            no_result = await page.query_selector(
                ".noResult, .is-noResult, [class*='no-result']"
            )
            if no_result:
                logger.debug(f"[yahoo] No results for {building_name}")
                return results

            cards = await page.query_selector_all(
                ".ListItem, .Property, [class*='property-card'], [class*='list-item']"
            )
            ...
            logger.info(
                f"[yahoo] {building_name} ({ward}): {len(results)} listings found"
            )
        except Exception as e:
            logger.error(f"[yahoo] Error searching {building_name}: {e}")
        finally:
            await page.close()
    ...
    async def _parse_card(self, card, building_name: str) -> Optional[ListingResult]:
        name_el = await card.query_selector(
            ".ListItem__name, [class*='building-name'], [class*='property-name']"
        )
        if name_el:
        ...
        link = await card.query_selector("a[href*='realestate.yahoo.co.jp']")
        if not link:
        ...
        if not href:
            return None
        url = href if href.startswith("http") else f"https://realestate.yahoo.co.jp{href}"
